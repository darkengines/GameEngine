   ...
   public:
          ...
          skinningPipeline(skinningPipeline)
    {
      auto skinnedMeshInstances =
          registry.view<Components::SkinnedMeshInstance, Nodes::Components::NodeMesh, Components::SkinnedBufferView>(
    ...
    void storeMeshes()
    {
      auto view = registry.view<Nodes::Components::NodeMesh, Animations::Components::Skinned>(
          entt::exclude<Components::SkinnedBufferView>);
      std::vector<Devices::BufferView> vertexBufferViews;
      std::vector<entt::entity> entities;
      view.each(
          [&](entt::entity objectMeshEntity, const Nodes::Components::NodeMesh& nodeMesh)
          {
    ...
    void createSkinnedMeshInstanceResources(uint32_t frameIndex)
    {
      ...
      skinnedMeshInstances.each(
              ...
              const Components::SkinnedBufferView& skinnedBufferView)
          {
            ...
            std::transform(skinnedMeshInstance.skinnedVertices.begin(),
                ...
                {
                  const auto& [boneInstanceStoreItem, boneSpatialStoreItem] =
                      registry.get<Stores::StoreItem<Nodes::Models::Node>, Stores::StoreItem<Models::BoneSpatial>>(
                          vertexWeightInstance.boneInstanceEntity);
                  const auto& spatial = registry.get<Spatials::Components::Spatial<Spatials::Components::Relative>>(
                      vertexWeightInstance.boneInstanceEntity);
                  auto boneStoreItem = registry.get<Stores::StoreItem<Models::BoneMesh>>(vertexWeightInstance.boneEntity);
                  return Models::BoneInstanceWeight{ .boneInstanceStoreItemLocation =
                                                         boneInstanceStoreItem.frameStoreItems[frameIndex],
      ...
      if (!boneInstanceWeightBuffers.empty())
      {
        ...
        std::transform(boneInstanceWeightBuffers.begin(),
            boneInstanceWeightBuffers.end(),
            boneInstanceWeightBufferSpans.data(),
            [](auto& boneInstanceWeights) { return std::span(boneInstanceWeights.begin(), boneInstanceWeights.end()); });
        auto boneInstanceWeightBufferViews =
            animationResourceManager.createBoneInstanceWeightBufferViews(boneInstanceWeightBufferSpans);

        for (auto entityIndex = 0; entityIndex < entities.size(); entityIndex++)
        {
          registry.emplace<Components::BoneInstanceWeightBufferView>(
              entities[entityIndex], boneInstanceWeightBufferViews[entityIndex]);
        }

        auto skinnedWeightedMeshInstances = registry.view<Components::SkinnedMeshInstance,
            Nodes::Components::NodeMesh,
            Components::SkinnedBufferView,
            Components::BoneInstanceWeightBufferView>(entt::exclude<Components::SkinnedVertexRangeBufferView>);
        ...
        skinnedWeightedMeshInstances.each(
                ...
                const Components::BoneInstanceWeightBufferView& boneInstanceWeightBufferView)
            {
              ...
              std::transform(skinnedMeshInstance.skinnedVertexRanges.begin(),
                  ...
                  {
                    return Models::SkinnedVertexRange{ .vertexBufferIndex = skinnedBufferView.bufferArrayElement,
                      .vertexBufferItemOffset =
                          static_cast<uint32_t>(skinnedBufferView.bufferView.byteOffset / sizeof(Meshes::Vertex)),
                      ...
                      .skinnedVertexBufferItemOffset = static_cast<uint32_t>(
                          skinnedBufferView.frameSkinnedBufferViews[0].byteOffset / sizeof(Meshes::Vertex)),
                      ...
                      .vertexWeightBufferItemOffset = static_cast<uint32_t>(
                          boneInstanceWeightBufferView.bufferView.byteOffset / sizeof(Models::BoneInstanceWeight)),
        ...
        std::transform(skinnedVertexRangeBuffers.begin(),
        ...
        for (auto entityIndex = 0; entityIndex < entities.size(); entityIndex++)
        {
          registry.emplace<Components::SkinnedVertexRangeBufferView>(
              entities[entityIndex], result[entityIndex].bufferView, result[entityIndex].bufferIndex);
        }
      }
    }
    ...
    void updateSkins(vk::CommandBuffer commandBuffer)
    {
      ...
      view.each(
          [&commandBuffer, this](
              entt::entity entity, const Components::SkinnedVertexRangeBufferView& skinnedVertexRangeBufferView)
          {
            auto rangeCount = static_cast<uint32_t>(
                skinnedVertexRangeBufferView.bufferView.byteLength / sizeof(Models::SkinnedVertexRange));
            auto itemOffset = static_cast<uint32_t>(
                skinnedVertexRangeBufferView.bufferView.byteOffset / sizeof(Models::SkinnedVertexRange));
    ...
    {
      auto skinnedObjectMeshView =
          registry.view<Animations::Components::AnimationState, Animations::Components::NodeAnimation>();
      skinnedObjectMeshView.each(
          [this](entt::entity objectMeshEntity,
              Animations::Components::AnimationState& animationState,
              const Animations::Components::NodeAnimation& nodeAnimation)
          {
            if (nodeAnimation.nodeEntity == entt::null)
              return;
            const Components::Animation& animation =
                registry.get<Animations::Components::Animation>(nodeAnimation.animationEntity);
        ...
        glm::vec3& scaling)
    {
      ...
      {
        auto positionUpperBound = std::upper_bound(nodeAnimation.positionKeys.begin(),
            nodeAnimation.positionKeys.end(),
            time,
            [](double time, const auto& positionKey) { return time < positionKey.time; });
        ...
        else if (positionUpperBound != nodeAnimation.positionKeys.begin())
        {
          ...
          position =
              currentPosition.vector + (nextPosition.vector - currentPosition.vector) * (float)positionTransitionProgress;
        }
      }
      else
      {
      ...
      {
        auto rotationUpperBound = std::upper_bound(nodeAnimation.rotationKeys.begin(),
            nodeAnimation.rotationKeys.end(),
            time,
            [](double time, const auto& rotationKey) { return time < rotationKey.time; });
      ...
      {
        auto scaleUpperBound = std::upper_bound(nodeAnimation.scalingKeys.begin(),
            nodeAnimation.scalingKeys.end(),
            time,
            [](double time, const auto& scaleKey) { return time < scaleKey.time; });
