...
LoadResult AssimpLoader::Load(std::filesystem::path scenePath, entt::registry &registry) const {
	...
	auto aiScene = importer.ReadFile(scenePath.string(),
									 aiProcess_FindInstances | aiProcess_GenBoundingBoxes | aiProcess_Triangulate | aiProcess_FindInstances | aiProcess_GenNormals | aiProcess_JoinIdenticalVertices | aiProcess_CalcTangentSpace | aiProcess_OptimizeMeshes | aiProcess_PopulateArmatureData);
...
void AssimpLoader::loadMaterials(std::span<aiMaterial *> aiMaterials,
	...
	for (auto aiMaterialIndex = 0u; aiMaterialIndex < aiMaterials.size(); aiMaterialIndex++) {
		...
		auto hasTransparency =
			(baseColorTexture != entt::null && registry.get<Textures::ImageInfo>(baseColorTexture).depth > 3) || (ambientColorTexture != entt::null && registry.get<Textures::ImageInfo>(ambientColorTexture).depth > 3) || (diffuseColorTexture != entt::null && registry.get<Textures::ImageInfo>(diffuseColorTexture).depth > 3) || (hasAmbientColor && ambientColor.a < 1) || (hasDiffuseColor && diffuseColor.a < 1);
...
void AssimpLoader::postProcessSkinnedMeshes(entt::registry &registry) {
	registry.sort<Animations::Components::NodeBoneNodeMesh>(
		[](const Animations::Components::NodeBoneNodeMesh &left, const Animations::Components::NodeBoneNodeMesh &right) { return left.nodeMeshEntity < right.nodeMeshEntity; });
	...
	for (const auto &aiAnimation : aiAnimations) {
		...
		std::transform(aiChannels.begin(),
					   ...
					   [&nodeNameAnimationMap, &loadResult, &registry, animationEntity](const aiNodeAnim *aiChannel) {
						   ...
						   nodeNameAnimationMap[loadResult.getEntityExternalId(std::string(aiChannel->mNodeName.C_Str()))] =
							   nodeAnimationEntity;
	...
	for (auto aiLight : aiLights) {
		...
		if (aiLight->mType == aiLightSourceType::aiLightSource_POINT) {
			...
			Lights::Components::LightPerspectiveCollection lightPerspectiveCollection{
				.lightPerspectives = {frontLightPerspectiveEntity,
...
entt::entity AssimpLoader::loadNode(const aiNode *assimpNode,
	...
	auto nodeAnimationEntityEntry =
		aiNodeNameNodeAnimationMap.find(loadResult.getEntityExternalId(std::string(assimpNode->mName.C_Str())));
	...
	auto boneEntityEntry =
		aiBonePtrBoneEntityMap.find(loadResult.getEntityExternalId(std::string(assimpNode->mName.C_Str())));
