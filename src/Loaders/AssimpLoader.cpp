...
namespace drk::Loaders
{
  ...
  LoadResult AssimpLoader::Load(std::filesystem::path scenePath, entt::registry& registry) const
  {
    ...
    auto aiScene = importer.ReadFile(scenePath.string(),
        aiProcess_FindInstances | aiProcess_GenBoundingBoxes | aiProcess_Triangulate | aiProcess_FindInstances |
            aiProcess_GenNormals | aiProcess_JoinIdenticalVertices | aiProcess_CalcTangentSpace | aiProcess_OptimizeMeshes |
    ...
    auto rootEntity = loadNode(aiScene->mRootNode,
        lightMap,
        cameraMap,
  ...
  void AssimpLoader::loadMaterials(std::span<aiMaterial*> aiMaterials,
      ...
      entt::registry& registry) const
  {
    ...
    for (auto aiMaterialIndex = 0u; aiMaterialIndex < aiMaterials.size(); aiMaterialIndex++)
    {
          ...
          if (result == aiReturn::aiReturn_SUCCESS)
          {
            ...
            if (texturePathPair == textureNameMap.end())
            {
              ...
              else
              {
                ...
                image = std::move(Textures::ImageInfo::fromFile(
                    textureFileSystemPath.string(), textureFileSystemPath.string(), textureTypePair.second));
              }
              if (image.pixels.size() > 0)
              {
                auto textureEntity = registry.create();
                registry.emplace<Common::Components::Name>(
                    textureEntity, std::filesystem::path(texturePath).filename().string());
        ...
        else
        {
          ...
          if (regexEntry != TextureTypeRegexMap.end())
          {
            for (const auto& entry : std::filesystem::directory_iterator(workingDirectoryPath))
            {
              ...
              if (ok)
              {
                auto image = std::move(Textures::ImageInfo::fromFile(
                    entry.path().filename().string(), entry.path().string(), textureTypePair.second));
                if (image.pixels.size() > 0)
                {
                  auto textureEntity = registry.create();
      ...
      entt::entity baseColorTexture =
          baseColorTexturePair != textureTypeMap.end() ? baseColorTexturePair->second : entt::null;
      entt::entity ambientColorTexture =
          ambientColorTexturePair != textureTypeMap.end() ? ambientColorTexturePair->second : entt::null;
      entt::entity diffuseColorTexture =
          diffuseColorTexturePair != textureTypeMap.end() ? diffuseColorTexturePair->second : entt::null;
      entt::entity specularColorTexture =
          specularColorTexturePair != textureTypeMap.end() ? specularColorTexturePair->second : entt::null;
      ...
      entt::entity metallicRoughnessTexture =
          metallicRoughnessPair != textureTypeMap.end() ? metallicRoughnessPair->second : entt::null;
      ...
      auto hasTransparency =
          (baseColorTexture != entt::null && registry.get<Textures::ImageInfo>(baseColorTexture).depth > 3) ||
          (ambientColorTexture != entt::null && registry.get<Textures::ImageInfo>(ambientColorTexture).depth > 3) ||
          (diffuseColorTexture != entt::null && registry.get<Textures::ImageInfo>(diffuseColorTexture).depth > 3) ||
          (hasAmbientColor && ambientColor.a < 1) || (hasDiffuseColor && diffuseColor.a < 1);

      Materials::Components::Material material = { .baseColor = glm::vec4(
  ...
  void AssimpLoader::postProcessSkinnedMeshes(entt::registry& registry)
  {
    registry.sort<Animations::Components::NodeBoneNodeMesh>(
        [](const Animations::Components::NodeBoneNodeMesh& left, const Animations::Components::NodeBoneNodeMesh& right)
    ...
    skinnedNodeMeshes.each(
        [&previousMeshInstanceEntity, &skinnedMeshInstances, &registry](
            entt::entity skinnedNodeMeshEntity, Animations::Components::NodeBoneNodeMesh& nodeBoneNodeMesh)
        {
    ...
    for (const auto& skinnedMeshInstance : skinnedMeshInstances)
    {
      ...
      for (const auto& instance : skinnedMeshInstance.second)
      {
        ...
        {
          instanceSkinnedVertices.emplace_back(vertexWeight.vertexIndex,
              vertexWeight.weight,
      ...
      std::sort(instanceSkinnedVertices.begin(),
          instanceSkinnedVertices.end(),
          [](const Animations::Components::VertexWeightInstance& left,
              const Animations::Components::VertexWeightInstance& right) { return left.vertexIndex < right.vertexIndex; });
      ...
      registry.emplace<Animations::Components::SkinnedMeshInstance>(
          skinnedMeshInstance.first, std::move(instanceVertexWeightRanges), std::move(instanceSkinnedVertices));
    }
  }
  void AssimpLoader::loadMeshes(std::span<aiMesh*> aiMeshes,
      LoadResult& loadResult,
      entt::registry& registry,
      std::unordered_map<std::string, entt::entity>& aiBoneNodeNameBoneEntityMap,
      std::unordered_map<aiBone*, entt::entity>& aiBonePtrBoneEntityMap) const
  {
    for (auto aiMeshIndex = 0u; aiMeshIndex < aiMeshes.size(); aiMeshIndex++)
    {
      ...
      if (aiMesh->HasBones())
      {
        ...
        for (const auto& aiBone : aiBones)
        {
          ...
          Animations::Components::BoneMesh boneMesh{ .boneEntity = boneEntity,
      ...
      {
        memcpy(indices.data() + faceIndex * 3,
      ...
      for (uint32_t vertexIndex = 0; vertexIndex < aiMesh->mNumVertices; vertexIndex++)
      {
        ...
        vertex.position = {
          aiMesh->mVertices[vertexIndex].x, aiMesh->mVertices[vertexIndex].y, aiMesh->mVertices[vertexIndex].z, 1
        };
        vertex.normal = {
          aiMesh->mNormals[vertexIndex].x, aiMesh->mNormals[vertexIndex].y, aiMesh->mNormals[vertexIndex].z, 0
        };
        vertex.tangent = aiMesh->mTangents != nullptr ? glm::vec4{ aiMesh->mTangents[vertexIndex].x,
          aiMesh->mTangents[vertexIndex].y,
          aiMesh->mTangents[vertexIndex].z,
          0 }
                                                      : glm::vec4{ 0, 0, 0, 0 };
        vertex.bitangent = aiMesh->mBitangents != nullptr ? glm::vec4{ aiMesh->mBitangents[vertexIndex].x,
      ...
      Meshes::Components::MeshResource mesh = { .vertices = vertices,
        .indices = indices,
  ...
  void AssimpLoader::loadSkeletons(std::span<aiSkeleton*> aiSkeletons,
      LoadResult& loadResult,
      entt::registry& registry) const
  {
  ...
  void AssimpLoader::loadAnimations(std::span<aiAnimation*> aiAnimations,
      ...
      entt::registry& registry) const
  {
    for (const auto& aiAnimation : aiAnimations)
    {
      ...
      Animations::Components::Animation animation{ .duration = aiAnimation->mDuration,
  ...
  void AssimpLoader::loadLights(std::span<aiLight*> aiLights,
      ...
      LoadResult& loadResult) const
  {
    for (auto aiLight : aiLights)
    {
      ...
      if (aiLight->mType == aiLightSourceType::aiLightSource_POINT)
      {
        ...
        Spatials::Components::Spatial<Spatials::Components::Absolute> lightAbsoluteSpatial{
          .position = { aiLight->mPosition.x, aiLight->mPosition.y, aiLight->mPosition.z, 1 }
        };

        Lights::Components::LightPerspective frontLightPerspective = { .relativeFront = GlmExtensions::front,
        ...
        Lights::Components::LightPerspective backLightPerspective = { .relativeFront = GlmExtensions::back,
          .relativeUp = GlmExtensions::up,
          .verticalFov = glm::half_pi<float>(),
          .aspectRatio = 1.0f,
          .near = 0.001f,
        ...
        Lights::Components::LightPerspective leftLightPerspective = { .relativeFront = GlmExtensions::left,
          .relativeUp = GlmExtensions::up,
          .verticalFov = glm::half_pi<float>(),
          .aspectRatio = 1.0f,
          .near = 0.001f,
        ...
        Lights::Components::LightPerspective rightLightPerspective = { .relativeFront = GlmExtensions::right,
          .relativeUp = GlmExtensions::up,
          .verticalFov = glm::half_pi<float>(),
          .aspectRatio = 1.0f,
          .near = 0.001f,
        ...
        Lights::Components::LightPerspective upLightPerspective = { .relativeFront = GlmExtensions::up,
        ...
        Lights::Components::LightPerspective downLightPerspective = { .relativeFront = GlmExtensions::down,
        ...
        registry.emplace<Lights::Components::LightPerspective>(
            frontLightPerspectiveEntity, std::move(frontLightPerspective));
        ...
        registry.emplace<Lights::Components::LightPerspective>(
            rightLightPerspectiveEntity, std::move(rightLightPerspective));
        ...
        Lights::Components::LightPerspectiveCollection lightPerspectiveCollection{ .lightPerspectives = {
        ...
        registry.emplace<Spatials::Components::Spatial<Spatials::Components::Absolute>>(entity, lightAbsoluteSpatial);
      ...
      entt::registry& registry) const
  {
    for (auto aiCamera : aiCameras)
    {
      ...
      auto perspective =
          glm::perspectiveZO(aiCamera->mHorizontalFOV, aiCamera->mAspect, aiCamera->mClipPlaneNear, aiCamera->mClipPlaneFar);
      ...
      Cameras::Components::Camera camera{ perspective,
      ...
      auto cameraFrustum = Frustums::Components::Frustum::createFrustumFromView(glm::vec4(cameraPosition, 1.f),
          glm::vec4(front, 0.f),
  ...
  entt::entity AssimpLoader::loadNode(const aiNode* assimpNode,
      ...
      int depth) const
  {
      ...
      for (auto boneMeshEntity : bone.boneMeshEntities)
      {
        ...
        if (map.contains(boneMesh.skinnedMeshEntity))
        {
          ...
          registry.emplace<Animations::Components::NodeBoneNodeMesh>(
              nodeBoneMeshEntity, entity, rootBoneInstanceEntity, nodeMeshEntity, boneMeshEntity);
        }
      }
      registry.emplace<Animations::Components::RootBoneInstanceReference>(entity, rootBoneInstanceEntity);
      Spatials::Components::Spatial<Animations::Components::Bone> boneSpatial{ position, rotation, scale, localModel };
      registry.get_or_emplace<Spatials::Components::Spatial<Animations::Components::Bone>>(entity, boneSpatial);
    ...
    if (assimpNode->mNumChildren)
    {
      ...
      for (auto childIndex = 0u; childIndex < assimpNode->mNumChildren; childIndex++)
      {
        ...
        auto childEntity = loadNode(aiChildNode,
            lightMap,
            cameraMap,
  ...
  glm::vec3& AssimpLoader::toVector(const aiVector3D& aiVector)
  {
    return (*(glm::vec3*)&aiVector);
  }
  [[maybe_unused]] glm::vec4 AssimpLoader::toVector4(const aiVector3D& aiVector, float w)
  {
    return glm::vec4(aiVector.x, aiVector.y, aiVector.z, w);
  }
}  // namespace drk::Loaders
