...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Call ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ about ImTextureID! See
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large meshes support (64k+ vertices) even with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset).
...
//  2025-02-13: Vulkan: Added ApiVersion field in ImGui_ImplVulkan_InitInfo. Default to header version if unspecified. Dynamic rendering path loads
//  "vkCmdBeginRendering/vkCmdEndRendering" (without -KHR suffix) on API 1.3. (#8326) 2025-01-09: Vulkan: Added IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how
//  many image sampler descriptors are expected to be available in descriptor pool. (#6642) 2025-01-06: Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to simplify our
//  examples. 2024-12-11: Vulkan: Fixed setting VkSwapchainCreateInfoKHR::preTransform for platforms not supporting VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27:
//  Vulkan: Make user-provided descriptor pool optional. As a convenience, when setting init_info->DescriptorPoolSize the backend will create one itself. (#8172, #4867) 2024-10-07:
//  Vulkan: Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: Vulkan: Expose selected render state in ImGui_ImplVulkan_RenderState, which you can access
//  in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: Vulkan: Compiling with '#define ImTextureID=ImU64' is unnecessary now that dear imgui defaults
//  ImTextureID to u64 instead of void*. 2024-04-19: Vulkan: Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter from ImGui_ImplVulkan_Init()
//  function to ImGui_ImplVulkan_InitInfo structure. Not required when using dynamic rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling
//  PipelineRenderingCreateInfo structure. 2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one extra semaphore than in-flight
//  frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using full buffer size (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added
//  MinAllocationSize field in ImGui_ImplVulkan_InitInfo to workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stopped creating command pools
//  with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them. 2023-11-29: Vulkan: Fixed mismatching allocator passed to vkCreateCommandPool() vs
//  vkDestroyCommandPool(). (#7075) 2023-11-10: *BREAKING CHANGE*: Removed parameter from ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own command-buffer to
//  upload fonts.
...
//  2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define ImTextureID
//  ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a full-viewport to reduce likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their
//  app without calling vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized backend to pull data from a single structure to facilitate usage with multiple-contexts
//  (all g_XXXX access changed to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes are not multiple of
//  VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change blending equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added support for custom
//  function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for specifying which subpass to reference during
//  VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline parameter to ImGui_ImplVulkan_RenderDrawData (default to one passed to ImGui_ImplVulkan_Init). 2020-05-04: Vulkan:
//  Fixed crash if initial frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData didn't have vertices. 2019-08-01:
//  Vulkan: Added support for specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to use, default is
...
static inline VkDeviceSize AlignBufferSize(VkDeviceSize size, VkDeviceSize alignment) { return (size + alignment - 1) & ~(alignment - 1); }
...
static void ImGui_ImplVulkan_CreatePipeline(
	...
	if (bd->VulkanInitInfo.UseDynamicRendering) {
		IM_ASSERT(
			bd->VulkanInitInfo.PipelineRenderingCreateInfo.sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR &&
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(uint32_t api_version, PFN_vkVoidFunction (*loader_func)(const char* function_name, void* user_data), void* user_data) {
	...
	ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR =
		reinterpret_cast<PFN_vkCmdBeginRenderingKHR>(loader_func(api_version < VK_API_VERSION_1_3 ? "vkCmdBeginRenderingKHR" : "vkCmdBeginRendering", user_data));
	ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR =
		reinterpret_cast<PFN_vkCmdEndRenderingKHR>(loader_func(api_version < VK_API_VERSION_1_3 ? "vkCmdEndRenderingKHR" : "vkCmdEndRendering", user_data));
}
#endif
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
	...
	if (info->UseDynamicRendering) {
		...
		ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
			info->ApiVersion, [](const char* function_name, void* user_data) { return vkGetInstanceProcAddr((VkInstance)user_data, function_name); }, (void*)info->Instance
		);
#endif
...
// FIXME: This is experimental in the sense that we are unsure how to best design/tackle this problem, please post to https://github.com/ocornut/imgui/pull/914 if you have
// suggestions.
...
void ImGui_ImplVulkan_DestroyWindowRenderBuffers(VkDevice device, ImGui_ImplVulkan_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator) {
	for (uint32_t n = 0; n < buffers->Count; n++) ImGui_ImplVulkan_DestroyFrameRenderBuffers(device, &buffers->FrameRenderBuffers[n], allocator);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
	...
	for (uint32_t i = 0; i < wd->ImageCount; i++) ImGui_ImplVulkanH_DestroyFrame(device, &wd->Frames[i], allocator);
	for (uint32_t i = 0; i < wd->SemaphoreCount; i++) ImGui_ImplVulkanH_DestroyFrameSemaphores(device, &wd->FrameSemaphores[i], allocator);
	wd->Frames.clear();
...
#endif
	const VkFormat defaultFormats[] = {VK_FORMAT_B8G8R8A8_SRGB,
		VK_FORMAT_B8G8R8A8_UNORM,
		VK_FORMAT_R8G8B8A8_UNORM,
		VK_FORMAT_B8G8R8_UNORM,
		VK_FORMAT_R8G8B8_UNORM
};
	for (VkFormat format : defaultFormats) requestSurfaceImageFormats.push_back(format);

	const VkColorSpaceKHR requestSurfaceColorSpace = VK_COLORSPACE_SRGB_NONLINEAR_KHR;
	wd->SurfaceFormat =
		ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice, wd->Surface, requestSurfaceImageFormats.Data, (size_t)requestSurfaceImageFormats.Size, requestSurfaceColorSpace);
