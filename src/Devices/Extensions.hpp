...
{
  fruit::Component<DeviceContext> addDevices()
  {
    return fruit::createComponent()
        .registerConstructor<DeviceContext(
            const Configuration::Configuration& configuration, const Windows::Window& window)>()
        .install(Windows::addWindows)
        .install(Configuration::addConfiguration);
  }
  static auto AddDevices()
  {
    return boost::di::make_injector(boost::di::bind<DeviceContext>.to<DeviceContext>());
  }
}  // namespace drk::Devices
