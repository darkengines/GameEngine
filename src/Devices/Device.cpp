...
bool Device::checkDeviceExtensionSupport(const vk::PhysicalDevice& physicalDevice, const std::vector<const char*>& requiredExtensions) {
	auto availableExtensions = physicalDevice.enumerateDeviceExtensionProperties();
	auto hasAllRequiredExtensions = std::all_of(requiredExtensions.begin(), requiredExtensions.end(), [&availableExtensions](const char* requiredExtension) {
		auto foundExtension = std::find_if(availableExtensions.begin(), availableExtensions.end(), [&requiredExtension](const vk::ExtensionProperties& availableExtension) {
...
bool Device::isDeviceSuitable(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, const std::vector<const char*>& requiredExtensions) {
	...
	return indices.isComplete() && extensionsSupported && swapChainAdequate && supportedFeatures.features.samplerAnisotropy && indexingFeatures.runtimeDescriptorArray &&
		   indexingFeatures.descriptorBindingPartiallyBound;
}

QueueFamilyIndices Device::findQueueFamilies(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface) {
...
SwapChainSupportDetails Device::querySwapChainSupport(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface) {
...
vk::PhysicalDevice Device::pickPhysicalDevice(const vk::Instance& instance, const vk::SurfaceKHR& surface, const std::vector<const char*>& requiredExtensions) {
	...
	if (suitablePhysicalDevices.size() > 0)
		return suitablePhysicalDevices[suitablePhysicalDevices.size() - 1];
...
VulkanLogicalDeviceInfo Device::createLogicalDevice(
	...
	for (uint32_t queueFamily : uniqueQueueFamilies) {
		vk::DeviceQueueCreateInfo queueCreateInfo = {.queueFamilyIndex = queueFamily, .queueCount = 1, .pQueuePriorities = &queuePriority};
	...
	return {.device = device, .graphicQueue = graphicQueue, .presentQueue = presentQueue, .computeQueue = computeQueue};
}

vk::Instance Device::createInstance(const std::vector<const char*>& requiredInstanceExtensions, const std::vector<const char*>& requiredLayers) {
	...
	vk::ValidationFeaturesEXT validationFeaturesEXT = {.enabledValidationFeatureCount = 1, .pEnabledValidationFeatures = &enabledValidationFeature};
...
vk::Format Device::findSupportedFormat(vk::PhysicalDevice physicalDevice, std::vector<vk::Format> candidates, vk::ImageTiling tiling, vk::FormatFeatureFlags features) {
...
VmaAllocator Device::createAllocator(const vk::Instance& instance, const vk::PhysicalDevice& physicalDevice, const vk::Device& device) {
	VmaVulkanFunctions vulkanFunctions = {.vkGetInstanceProcAddr = &vkGetInstanceProcAddr, .vkGetDeviceProcAddr = &vkGetDeviceProcAddr};
...
Buffer Device::createVmaBuffer(
	...
	auto result = vmaCreateBuffer(allocator, (VkBufferCreateInfo*)&bufferCreateInfo, pAllocationCreationInfo, (VkBuffer*)&buffer, &allocation, &allocationInfo);
...
Buffer Device::createBuffer(
	const VmaAllocator& allocator, vk::MemoryPropertyFlags properties, vk::BufferUsageFlags usage, const VmaAllocationCreateInfo& vmaAllocationCreateInfo, vk::DeviceSize size,
	const char* name
) {
	auto buffer = Device::createVmaBuffer(allocator, usage, &vmaAllocationCreateInfo, size, name);
	return buffer;
}

void Device::destroyVmaBuffer(const VmaAllocator& allocator, Buffer buffer) { vmaDestroyBuffer(allocator, (VkBuffer)buffer.buffer, buffer.allocation); }

void Device::destroyBuffer(const VmaAllocator& allocator, const Buffer& buffer) { Device::destroyVmaBuffer(allocator, buffer); }
...
void Device::copyBuffer(
	...
	vk::BufferCopy region = {.srcOffset = sourceOffset, .dstOffset = destinationOffset, .size = length};
...
void Device::mapBuffer(const VmaAllocator& allocator, const Buffer& buffer, void** memory) { vmaMapMemory(allocator, buffer.allocation, memory); }

void Device::unmapBuffer(const VmaAllocator& allocator, const Buffer& buffer) { vmaUnmapMemory(allocator, buffer.allocation); }
...
void Device::endSingleTimeCommands(const vk::Device& device, const vk::Queue& queue, const vk::CommandPool& commandPool, const vk::CommandBuffer& commandBuffer) {
	commandBuffer.end();
...
Image Device::createImage(const VmaAllocator& allocator, const vk::ImageCreateInfo imageCreationInfo, vk::MemoryPropertyFlags properties) {
...
Image Device::createVmaImage(const VmaAllocator& allocator, const vk::ImageCreateInfo& imageCreationInfo, const VmaAllocationCreateInfo& allocationCreationInfo) {
	VkImage image;
	VmaAllocation allocation;
	VmaAllocationInfo allocationInfo;

	auto result = vmaCreateImage(allocator, (VkImageCreateInfo*)&imageCreationInfo, &allocationCreationInfo, &image, &allocation, &allocationInfo);
	if (result != VkResult::VK_SUCCESS)
		throw std::runtime_error("Failed to create image.");
	return {(vk::Image)image, allocation, allocationInfo};
}

void Device::destroyVmaImage(const VmaAllocator& allocator, const Image& image) { vmaDestroyImage(allocator, (VkImage)image.image, image.allocation); }

void Device::destroyImage(const vk::Device& device, const VmaAllocator& allocator, const Image& image) { Device::destroyVmaImage(allocator, image); }
...
void Device::destroySampler(const vk::Device& device, const vk::Sampler& sampler) { device.destroySampler(sampler); }
...
vk::Extent2D Device::chooseSwapExtent(vk::Extent2D extent, const vk::SurfaceCapabilitiesKHR& capabilities) {
...
Swapchain Device::createSwapchain(const vk::Device& device, const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, const vk::Extent2D& extent) {
	...
	std::transform(swapchainImages.begin(), swapchainImages.end(), swapchainTextures.data(), [](const vk::Image& vkImage) { return Image{.image = vkImage}; });
	std::vector<vk::ImageView> swapchainImageViews(swapchainImages.size());
	std::transform(swapchainTextures.begin(), swapchainTextures.end(), swapchainImageViews.data(), [&device, surfaceFormat](const auto& swapchainImage) {
...
vk::ShaderModule Device::createShaderModules(const vk::Device& device, uint32_t codeSize, const uint32_t* pCode) {
	vk::ShaderModuleCreateInfo createInfo = {.codeSize = codeSize, .pCode = pCode};

	auto shaderModule = device.createShaderModule(createInfo);
	return shaderModule;
}

void Device::destroyShaderModule(const vk::Device& device, const vk::ShaderModule& shaderModule) { device.destroyShaderModule(shaderModule); }

void Device::copyBufferToImage(const vk::CommandBuffer& commandBuffer, const Devices::Buffer& source, const vk::Image& destination, const vk::BufferImageCopy& region) {
	commandBuffer.copyBufferToImage(source.buffer, destination, vk::ImageLayout::eTransferDstOptimal, 1, &region);
}
...
void Device::transitionLayout(
	...
	commandBuffer.pipelineBarrier(source, destination, {}, {}, {}, std::vector<vk::ImageMemoryBarrier>{barrier});
}

void Device::generatedMipmaps(const vk::CommandBuffer& commandBuffer, const vk::Image& image, int32_t width, int32_t height, uint32_t mipLevels) {
	vk::ImageMemoryBarrier barrier =
		{.image = image,
		 .subresourceRange = {
			 .aspectMask = vk::ImageAspectFlagBits::eColor,
