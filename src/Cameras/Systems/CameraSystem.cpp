...
namespace drk::Cameras::Systems
{
  ...
  {
    auto dirtyCameraView = registry.view<Components::Camera, Spatials::Components::Spatial<Spatials::Components::Relative>, Spatials::Components::Spatial<Spatials::Components::Absolute>, Common::Components::Dirty<Spatials::Components::Spatial<Spatials::Components::Relative>>>();
    dirtyCameraView.each(
        [&](entt::entity cameraEntity, Components::Camera& camera, Spatials::Components::Spatial<Spatials::Components::Relative>& relativeSpatial, Spatials::Components::Spatial<Spatials::Components::Absolute>& absoluteSpatial)
        {
          ...
          registry.emplace_or_replace<Graphics::SynchronizationState<Models::Camera>>(
              cameraEntity, static_cast<uint32_t>(engineState.getFrameCount()));
        });
  }
  ...
  entt::entity CameraSystem::createCamera(glm::vec4 position, glm::vec4 front, glm::vec4 up, float verticalFov, float aspectRatio, float near, float far) const
  {
    ...
    Components::Camera camera = { .position = glm::vec4(0.f, 0.f, 0.f, 1.f), .front = front, .up = up, .verticalFov = verticalFov, .aspectRatio = aspectRatio, .near = near, .far = far };
    ...
    Spatials::Components::Spatial<Spatials::Components::Absolute> cameraAbsoluteSpatial = { .position = position, .rotation = glm::quat(1, 0, 0, 0), .scale = glm::vec4(1.0f, 1.0f, 1.0f, 0.0f) };
    ...
    registry.emplace<Spatials::Components::Spatial<Spatials::Components::Absolute>>(cameraEntity, std::move(cameraAbsoluteSpatial));
