...
void FlyCamController::OnCursorPositionEvent(double xpos, double ypos) { MousePosition = glm::vec2(xpos, ypos); }
...
void FlyCamController::Step(float dt) {
	...
	if (hasPositionInputs) {
		...
		relativeSpatial.position += glm::vec4(relativeSpatial.rotation * (inputs * 10.0f * dt), 0);
		...
		if (LastMousePosition.has_value()) {
			...
			if (hasOrientationInputs) {
				if (roll > glm::half_pi<float>() || roll < -glm::half_pi<float>()) {
					yaw -= delta.x * dt * 0.066f;
				} else {
					yaw += delta.x * dt * 0.066f;
				}
				pitch += delta.y * dt * 0.066f;
			}
			relativeSpatial.rotation = glm::quat(glm::vec3(pitch, yaw, roll));
		}
