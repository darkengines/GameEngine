...
namespace drk::Meshes::Resources {
	...
	public:
		MeshResourceManager(Buffers::Resources::BufferResourceManager& bufferResourceManager) : bufferResourceManager(bufferResourceManager) {}
		MeshUploadResult uploadMeshes(const std::vector<std::shared_ptr<Components::MeshResource>>& meshInfos) {
			...
			std::transform(
				meshInfos.begin(), meshInfos.end(), vertices.data(), [](std::shared_ptr<Components::MeshResource> mesh) {
			...
			auto vertexBufferUploadResult = bufferResourceManager.upload(
				vertices,
				vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eTransferSrc
			);
