...
// #define JPH_ENABLE_ASSERTS false
...
namespace drk::Physics
{
  ...
  int test()
  {
    ...
    // calling this function. If you implement your own default material (PhysicsMaterial::sDefault) make sure to initialize
    ...
    drk::Physics::BroadPhaseLayer broad_phase_layer_interface;

    // Create class that filters object vs broadphase layers
    // Note: As this is an interface, PhysicsSystem will take a reference to this so this instance needs to stay alive!
    // Also have a look at ObjectVsBroadPhaseLayerFilterTable or ObjectVsBroadPhaseLayerFilterMask for a simpler interface.
    drk::Physics::ObjectVsBroadPhaseLayerFilter object_vs_broadphase_layer_filter;

    // Create class that filters object vs object layers
    // Note: As this is an interface, PhysicsSystem will take a reference to this so this instance needs to stay alive!
    // Also have a look at ObjectLayerPairFilterTable or ObjectLayerPairFilterMask for a simpler interface.
    drk::Physics::ObjectLayerPairFilter object_vs_object_layer_filter;
    ...
    drk::Physics::BodyActivationListener body_activation_listener;
    physics_system.SetBodyActivationListener(&body_activation_listener);

    // A contact listener gets notified when bodies (are about to) collide, and when they separate again.
    // Note that this is called from a job so whatever you do here needs to be thread safe.
    // Registering one is entirely optional.
    drk::Physics::ContactListener contact_listener;
    ...
    // non-locking variant of this. We're going to use the locking version (even though we're not planning to access bodies
    ...
    floor_shape_settings.SetEmbedded();  // A ref counted object on the stack (base class RefTarget) should be marked as such
    ...
    // detection performance (it's pointless here because we only have 2 bodies). You should definitely not call this every
    // frame or when e.g. streaming in a new level section as it is an expensive operation. Instead insert all new objects in
  ...
  fruit::Component<Systems::PhysicsSystem> addPhysics()
  {
    ...
    return fruit::createComponent()
        .registerConstructor<BodyActivationListener()>()
        .registerConstructor<BroadPhaseLayer()>()
        .registerConstructor<ContactListener()>()
        .registerConstructor<ObjectLayerPairFilter()>()
        .registerConstructor<ObjectVsBroadPhaseLayerFilter()>()
        .registerConstructor<Systems::PhysicsSystem(JPH::PhysicsSystem & joltPhysicsSystem,
            JPH::TempAllocatorImpl & tempAllocator,
            JPH::JobSystemThreadPool & jobSystemThreadPool)>()
        .registerProvider(
            [](const Configuration::Configuration &configuration)
            {
              const auto physicsConfiguration = configuration.jsonConfiguration["Physics"]
                      .get<PhysicsConfiguration>();
              return physicsConfiguration;
            })
        .registerProvider(
            [](const PhysicsConfiguration &physicsConfiguration)
            {
              // We need a temp allocator for temporary allocations during the physics update. We're
              // pre-allocating 10 MB to avoid having to do allocations during the physics update.
              // B.t.w. 10 MB is way too much for this example but it is a typical value you can use.
              // If you don't want to pre-allocate you can also use TempAllocatorMalloc to fall back to
              // malloc / free.
              auto allocator = new JPH::TempAllocatorImpl(physicsConfiguration.TempAllocatorSize);
              return allocator;
            })
        .registerProvider(
            []()
            {
              ...
              // you would implement the JobSystem interface yourself and let Jolt Physics run on top
              // of your own job scheduler. JobSystemThreadPool is an example implementation.
              auto jobSystemThreadPool = new JPH::JobSystemThreadPool(
                  JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, std::thread::hardware_concurrency() - 1);
        ...
        .registerProvider(
            ...
            {
              auto physicSystemPtr = new JPH::PhysicsSystem();
              physicSystemPtr->Init(physicsConfiguration.MaxBodyCount,
                  physicsConfiguration.BodyMutexCount,
                  physicsConfiguration.MaxBodyPairCount,
                  physicsConfiguration.MaxContactConstraintCount,
                  broadPhaseLayer,
                  objectVsBroadPhaseLayerFilter,
                  objectLayerPairFilter);
              return physicSystemPtr;
            })
