...
namespace drk::Applications
{
  ...
  Application::Application(const Windows::Window& window,
  ...
  void Application::taskflow()
  {
    ...
    auto controllerStepTask =
        taskflow.emplace([&]() { flyCamController.Step(applicationState.frameTime); }).name("ControllerSteps");
    ...
    auto lightPerspectiveDirtyTask =
        taskflow.emplace([&]() { lightPerspectiveSystem.processDirtyItems(); }).name("LightPerspectiveDirty");
    auto directionalLightDirtyTask =
        taskflow.emplace([&]() { directionalLightSystem.processDirtyItems(); }).name("DirectionalLightDirty");
    ...
    auto axisAlignedBoundingBoxDirtyTask =
        taskflow.emplace([&]() { axisAlignedBoundingBoxSystem.processDirty(); }).name("AxisAlignedBoundingBoxDirty");
    ...
    auto relativeSpatialStorageTask =
        taskflow.emplace([&]() { relativeSpatialSystem.store(); }).name("RelativeSpatialStorage");
    ...
    auto lightPerspectiveStorageTask =
        taskflow.emplace([&]() { lightPerspectiveSystem.store(); }).name("LightPerspectiveStorage");
    auto pointLightStorageTask = taskflow.emplace([&]() { pointLightSystem.store(); }).name("PointLightStorage");
    auto directionalLightStorageTask =
        taskflow.emplace([&]() { directionalLightSystem.store(); }).name("DirectionalLightStorage");
    auto spotlightStorageTask = taskflow.emplace([&]() { spotlightSystem.store(); }).name("SpotlightStorage");
    auto axisAlignedBoundingBoxStorageTask =
        taskflow.emplace([&]() { axisAlignedBoundingBoxSystem.store(); }).name("AxisAlignedBoundingBoxStorage");
    ...
    auto relativeSpatialStoreUpdateTask =
        taskflow.emplace([&]() { relativeSpatialSystem.store(); }).name("RelativeSpatialStoreUpdate");
    ...
    auto lightPerspectiveStoreUpdateTask =
        taskflow.emplace([&]() { lightPerspectiveSystem.store(); }).name("LightPerspectiveStoreUpdate");
    auto pointLightStoreUpdateTask = taskflow.emplace([&]() { pointLightSystem.store(); }).name("PointLightStoreUpdate");
    auto directionalLightStoreUpdateTask =
        taskflow.emplace([&]() { directionalLightSystem.store(); }).name("DirectionalLightStoreUpdate");
    auto spotlightStoreUpdateTask = taskflow.emplace([&]() { spotlightSystem.store(); }).name("SpotlightStoreUpdate");
    auto axisAlignedBoundingBoxStoreUpdateTask =
        taskflow.emplace([&]() { axisAlignedBoundingBoxSystem.store(); }).name("AxisAlignedBoundingBoxStoreUpdate");
    ...
    auto boneSpatialPropagationTask =
        taskflow.emplace([&]() { boneSpatialSystem.propagateChanges(); }).name("BoneSpatialPropagation");
    auto boneSpatialStorageTask = taskflow.emplace([&]() { boneSpatialSystem.store(); }).name("BoneSpatialStorage");
    auto boneSpatialStoreUpdateTask =
        taskflow.emplace([&]() { boneSpatialSystem.updateStore(); }).name("BoneSpatialStoreUpdate");

    auto skinnedMeshInstanceResourceCreationTask =
        taskflow.emplace([&]() { animationSystem.createSkinnedMeshInstanceResources(engineState.getFrameIndex()); })
            .name("SkinnedMeshInstanceResourceCreation");
    auto skinUpdateTask =
        taskflow.emplace([&]() { animationSystem.updateSkins(frameState.commandBuffer); }).name("SkinUpdates");
    ...
    auto axisAlignedBoundingBoxDrawsTask =
        taskflow.emplace([&]() { axisAlignedBoundingBoxSystem.emitDraws(); }).name("AxisAlignedBoundingBoxDraws");
  ...
  void Application::run()
  {
    ...
    //				.relativeScale = {1.0f, 1.0f, 1.0f, 1.0f},
    //				.relativeRotation = glm::quatLookAt(glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(1.0f, 0.0f,
    // 0.0f)), 				.relativePosition = glm::vec4(r, 1.0), 				.relativeModel =
    ...
    while (!glfwWindowShouldClose(window.GetWindow()))
    {
      ...
      //				ImGuizmo::SetRect(viewport->WorkPos.x, viewport->WorkPos.y, viewport->Size.x,
      // viewport->Size.y);
      //
      //				const auto& camera =
      // registry.get<Cameras::Components::Camera>(engineState.cameraEntity); 				auto perspective =
      // camera.perspective; 				perspective[1][1] *= -1.0f; 				ImGuizmo::DrawGrid( 					glm::value_ptr(camera.view),
      ...
      if (sceneTexture.has_value())
      {
        Devices::Device::transitionLayout(frameState.commandBuffer,
