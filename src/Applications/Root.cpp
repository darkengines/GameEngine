...
namespace drk::Applications {
	...
	void Root::renderGui(ApplicationState &applicationState, std::optional<ImVec2> &sceneCursorPosition) {
		...
		if (true || userInterface.IsVisible()) {
			...
			sceneCursorPosition = ImGui::GetCursorScreenPos();
	...
	void Root::updateApplicationState(std::optional<Devices::Texture> &sceneTexture,
		...
		if (shouldRecreateSwapchain || swapchainImageAcquisitionResult.result == vk::Result::eSuboptimalKHR || swapchainImageAcquisitionResult.result == vk::Result::eErrorOutOfDateKHR) {
		...
		if (applicationState.sceneExtent.width != applicationState.actualExtent.width || applicationState.sceneExtent.height != applicationState.actualExtent.height) {
			if ((applicationState.sceneExtent.width < applicationState.actualExtent.width && applicationState.actualExtent.width < INT_MAX) || (applicationState.sceneExtent.height < applicationState.actualExtent.height && applicationState.actualExtent.height < INT_MAX)) {
				if (applicationState.sceneTextureImageDescriptorSet.has_value())
	...
	void Root::run() {
		...
		std::optional<ImVec2> sceneCursorPosition;
		...
		const auto emitDrawsTask = rootTaskflow.emplace([&]() {
			for (const auto &drawSystem : drk::Draws::Systems::IDrawSystem::drawSystems) {
				drawSystem->emitDraws();
			}
			}).succeed(spatialSystemPropagation).succeed(lightPerspectiveUpdate);
		...
		*/
		...
		while (running = !glfwWindowShouldClose(window.GetWindow())) {
			...
			physicsSystem.addBodies(registry);
			physicsSystem.syncNodeToPhysics(registry);
			...
			physicsSystem.stepPhysics(applicationState.frameTime);
			physicsSystem.syncPhysicsToNode(registry);
